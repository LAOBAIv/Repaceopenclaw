...
export function ModelChannels() {
  ...
  const handleEdit = (ch: Channel) => {
    ...
    setForm({
      ...
      maxTokens: ch.maxTokens ?? 4096,
      topP: ch.topP ?? 1,
      frequencyPenalty: ch.frequencyPenalty ?? 0,
      presencePenalty: ch.presencePenalty ?? 0,
    });
    setShowForm(true);
    setShowPresets(false);
    setSaveMsg('');
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
          ...
          {showPresets && (
            <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <label style={labelStyle}>选择预设模型</label>
              <select 
                value={''}
                onChange={(e) => {
                  const value = e.target.value;
                  if (value !== '') {
                    applyPreset(parseInt(value));
                  }
                }}
                style={{
                  ...inputStyle,
      ...
      {showForm && (
        <div style={{ background: '#fff', border: '1.5px solid #2563eb', borderRadius: 12, padding: '20px 22px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            ...
            {/* 模型调用参数 */}
            <div style={{ gridColumn: '1 / -1', marginTop: 8, padding: '12px 14px', background: '#f8fafc', borderRadius: 8, border: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 10 }}>🎛️ 模型调用参数</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div>
                  <label style={labelStyle}>Temperature</label>
                  <input type="number" value={form.temperature}
                    onChange={e => setForm(f => ({ ...f, temperature: Number(e.target.value) }))}
                    style={inputStyle} min={0} max={2} step={0.1} />
                  <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>创造性 (0-2)</div>
                </div>
                <div>
                  <label style={labelStyle}>Max Tokens</label>
                  <input type="number" value={form.maxTokens}
                    onChange={e => setForm(f => ({ ...f, maxTokens: Number(e.target.value) }))}
                    style={inputStyle} min={1} max={128000} step={1} />
                  <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>最大输出长度</div>
                </div>
                <div>
                  <label style={labelStyle}>Top P</label>
                  <input type="number" value={form.topP}
                    onChange={e => setForm(f => ({ ...f, topP: Number(e.target.value) }))}
                    style={inputStyle} min={0} max={1} step={0.01} />
                  <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>核采样 (0-1)</div>
                </div>
                <div>
                  <label style={labelStyle}>Frequency Penalty</label>
                  <input type="number" value={form.frequencyPenalty}
                    onChange={e => setForm(f => ({ ...f, frequencyPenalty: Number(e.target.value) }))}
                    style={inputStyle} min={-2} max={2} step={0.1} />
                  <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>频率惩罚 (-2~2)</div>
                </div>
                <div>
                  <label style={labelStyle}>Presence Penalty</label>
                  <input type="number" value={form.presencePenalty}
                    onChange={e => setForm(f => ({ ...f, presencePenalty: Number(e.target.value) }))}
                    style={inputStyle} min={-2} max={2} step={0.1} />
                  <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>存在惩罚 (-2~2)</div>
                </div>
              </div>
            </div>
          </div>
