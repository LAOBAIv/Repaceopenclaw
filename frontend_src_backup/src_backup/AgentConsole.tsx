...
function AgentPicker({ agentList, selected, onClose, onConfirm }: {
  ...
  return (
    <Modal onClose={onClose}>
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.14)', width: 360, maxHeight: '70vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
...
function makeDefaultNode(): FlowNode {
  return { id: Date.now().toString(), name: '流程节点 1', nodeType: 'serial', agentIds: [], desc: '' };
}
...
export function AgentConsole() {
  ...
  const { addProject } = useProjectKanbanStore();
  ...
  async function handleCreate() {
    ...
    const dd = String(Number(String(now.getDate() + 30).padStart(2, '0'))).toString();
    addProject({
      id: `proj_${Date.now()}`,
      title: name,
      description: taskDesc.trim(),
      ...
      progress: 0,
      dueDate: endTime ? endTime.slice(0, 10).replace(/-/g, '/').slice(5) : `${mm}/${dd}`,
      updatedAt: '刚刚',
      taskCount: nodes.length,
      memberCount: nodes.reduce((s, n) => s + n.agentIds.length, 0) || 1,
    }, 'progress');
  ...
  return (
    <>
      <style>{`
        ...
        .pc-left {
          border-right: 1px solid #ebebeb; overflow-y: auto;
        ...
        <div className="pc-shell">
          ...
          <div className="pc-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <span style={{ fontWeight: 700, fontSize: 16, color: '#1a202c' }}>项目协作</span>
                <span style={{ fontSize: 12, color: '#9ca3af', marginLeft: 10 }}>配置多智能体协作流程</span>
              </div>
            </div>
          </div>
          ...
          <div className="pc-main">
            ...
            <div className="pc-left">
              ...
              <div className="pc-field">
                <label className="pc-label">项目名称<em>*</em></label>
                <input className="pc-input" placeholder="为本次协作项目起一个名字…"
                  value={taskName} onChange={e => setTaskName(e.target.value)} />
              </div>

              <div className="pc-field">
                <label className="pc-label">项目描述</label>
                <textarea className="pc-textarea" style={{ height: 76 }} placeholder="描述项目背景与预期产出…"
              ...
              <div className="pc-field">
                ...
                <textarea className="pc-textarea" style={{ height: 152 }} placeholder="本次项目期望达成的核心目标…"
                            ...
                            <div className={`pc-node${node.nodeType === 'parallel' ? ' parallel' : ''}`}>
                              ...
                              <div className="pc-node-bd">
                                <div className="pc-field" style={{ marginBottom: 8 }}>
                                  <label className="pc-label" style={{ fontSize: 12 }}>节点任务描述</label>
                                  <input className="pc-input" style={{ fontSize: 12 }} placeholder="描述该节点需要完成的任务…"
                                    value={node.desc} onChange={e => updateNode(node.id, { desc: e.target.value })} />
                                </div>
                                <div>
          ...
          <div className="pc-footer">
            <button className="pc-btn-cancel" onClick={handleReset}>取消</button>
