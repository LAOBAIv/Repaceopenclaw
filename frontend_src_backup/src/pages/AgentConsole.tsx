...
export function AgentConsole() {
  ...
  /* 当 editProject 存在且 agentList 加载完成时，将项目的智能体预填到第一个节点 */
  useEffect(() => {
  ...
  async function handleCreate() {
    ...
    /* ── 编辑项目 ── */
    if (editProject) {
      const projectAgents = allAgents.length >= 2
        ? allAgents
        : [
            { name: assignedAgent?.name ?? editProject.agent, color: assignedAgent?.color ?? editProject.agentColor },
            ...editProject.agents.filter((a: { name: string }) => a.name !== (assignedAgent?.name ?? editProject.agent)),
          ].slice(0, Math.max(2, allAgents.length));
    ...
    addProject({
      ...
      agents: allAgents.length >= 2
        ? allAgents
        : [
            { name: assignedAgent?.name ?? '策划助手', color: assignedAgent?.color ?? '#6366f1' },
            ...allAgents.filter(a => a.name !== (assignedAgent?.name ?? '策划助手')),
          ],
      progress: 0,
    ...
    handleReset();
    // 修复：传入 projectId，确保 workspace 以项目模式打开
    navigate('/workspace', { state: { projectName: name, projectId: newProjectId } });
  }
