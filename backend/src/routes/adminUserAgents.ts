...
router.get("/", ...adminOnly, (req: Request, res: Response) => {
  ...
  // 追加系统级智能体（不在 DB 中，运行时动态构建）
  const systemAgents = [
    AgentService.getByIdOrCode('repaceclaw-platform-assistant'),
    AgentService.getByIdOrCode('rc-wechat-agent'),
    ...
    modelProvider: agent!.modelProvider || "openclaw",
  ...
  res.json({ data: [...systemAgents, ...agents] });
});
