...
router.post("/", authenticate, requireRole(["super_admin", "admin"]), (req: Request, res: Response) => {
  ...
  const db = getDb();
  const { provider, modelName, baseUrl, apiKey, authType, enabled, priority, isPreset, temperature, maxTokens, topP, frequencyPenalty, presencePenalty } = parsed.data;
  ...
  if (existing.length && existing[0].values.length) {
    ...
    db.run(
      `UPDATE token_channels SET model_name=?, base_url=?, api_key=?, auth_type=?, enabled=?, priority=?, is_preset=?, temperature=?, max_tokens=?, top_p=?, frequency_penalty=?, presence_penalty=?, updated_at=? WHERE id=?`,
      [modelName, baseUrl, apiKey, authType, enabled ? 1 : 0, priority, isPreset ? 1 : 0, temperature, maxTokens, topP, frequencyPenalty, presencePenalty, now, id]
    );
    saveDb();
    res.json({ data: { id, provider, modelName, baseUrl, apiKey, authType, enabled, priority, isPreset, temperature, maxTokens, topP, frequencyPenalty, presencePenalty } });
  } else {
    const id = uuidv4();
    ...
    db.run(
      `INSERT INTO token_channels (id, provider, model_name, base_url, api_key, auth_type, enabled, priority, is_preset, temperature, max_tokens, top_p, frequency_penalty, presence_penalty, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, provider, modelName, baseUrl, apiKey, authType, enabled ? 1 : 0, priority, isPreset ? 1 : 0, temperature, maxTokens, topP, frequencyPenalty, presencePenalty, now, now]
    );
    saveDb();
    res.status(201).json({ data: { id, provider, modelName, baseUrl, apiKey, authType, enabled, priority, isPreset, temperature, maxTokens, topP, frequencyPenalty, presencePenalty } });
  }
});
