...
/**
 * GET /api/conversations/wechat-assistant
 * 获取或创建当前用户的微信助手专属会话
 * 
 * 改造日期：2026-05-12
 * 功能：
 *   - 每个用户有且仅有一个微信助手会话
 *   - 首次访问自动创建
 *   - 自动绑定 OC session key（幂等）
 * 
 * 返回参数：
 *   - id: 会话 UUID
 *   - agentIds: ['rc-wechat-agent' 或 DB UUID]
 *   - currentAgentId: 同上
 *   - title: '微信助手'
 *   - sessionCode: 业务会话编码
 *   - currentAgentCode: 业务智能体编码
 *   - messages: 历史消息列表
 *   - ocSessionKey: agent:rc-wechat-agent:rc:{convId}
 * 
 * 创建参数：
 *   - agentIds: [waAgent.id]
 *   - scopeType: 'user'
 *   - scopeId: userId
 *   - memoryPolicy: 'private'
 *   - ocSessionKey: agent:rc-wechat-agent:rc:{conv.id}
 */
router.get("/wechat-assistant", (req: Request, res: Response) => {
  const userId = (req as any).user?.id;
  if (!userId) return res.status(401).json({ error: '未认证' });
  ...
  if (existingList.length > 0) {
    ...
    // 关键修复：已有会话也要确保 oc_session_key 已绑定
    // 历史会话可能没有 oc_session_key，需要补上（bindOpenClawSession 是幂等的）
